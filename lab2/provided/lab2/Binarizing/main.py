...
# 0. Параметры:
TEST_IMG_FILE = os.path.join(
    os.path.dirname(__file__), "../data/tst_img.png"
...
fig = plt.figure()
Ierr = show.axes_init(
    [0.04, 0.75, 0.92, 0.20], "Ошибка 1-го рода (захват пикселей фона)", ticks="on"
)
IIerr = show.axes_init(
    [0.04, 0.40, 0.92, 0.20], "Ошибка 2-го рода (пропуск пикселей сигнала)", ticks="on"
)
